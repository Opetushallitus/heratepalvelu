(ns oph.heratepalvelu.tep.tpkNiputusHandler
...
(gen-class :name "oph.heratepalvelu.tep.tpkNiputusHandler"
...
(defn get-kausi-alkupvm [pvm]
  (str (.getYear pvm) (if (<= (.getMonthValue pvm) 6) "-01-01" "-07-01")))

(defn get-kausi-loppupvm [pvm]
...
;; TODO muista lisätä request-id muualla

;; TODO kun haetaan linkki Arvosta, lisätään request-id ja kyselylinkki, tunnus,
;; ja voimassa-loppupvm nippuun.

(defn save-tpk-nippu [nippu]
  (try
    (ddb/put-item
...
;; Jos edeltävän kauden vastausaika ei ole loppunut, niputtaa jaksoja
;; edeltävästä kaudesta. Muuten niputtaa nykyisen kauden jaksot.
