...
(deftest test-msg-body
  (testing "Varmista, että msg-body tekee formatointia oikein"
    (is (= (elisa/msg-body "kysely.linkki/123" [{:fi "Testilaitos"}
                                                {:fi "Testilaitos 2"}])
           mock-msg-body))))

(def mock-muistutus-msg-body
...
(deftest test-muistutus-msg-body
  (testing "Varmista, että muistutus-msg-body tekee formatointia oikein"
    (is (= (elisa/muistutus-msg-body
             "kysely.linkki/123"
             [{:fi "Testilaitos"} {:fi "Testilaitos 2"}])
           mock-muistutus-msg-body))))

(deftest test-send-tep-sms-send-messages-disabled
  (testing "send-tep-sms palauttaa oikean arvon, kun viestit eivät lähde"
...
(deftest test-send-tep-sms-no-error
  ...
  (testing "Varmista, että send-tep-sms käsittelee virheitä oikein"
    (with-redefs [clojure.tools.logging/log* tu/mock-log*
      ...
      (let [number "0401234567"
        ...
        (is (thrown? ExceptionInfo (elisa/send-tep-sms number message)))
